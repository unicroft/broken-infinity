...
public class SpriteManager : MonoBehaviour
{
    ...
    {
        SpriteAnimator tiledTexture = sprite.GetComponent<SpriteAnimator>();

        if (tiledTexture == null)
        {
            Debug.LogError("There is no SpriteAnimator script attached to the game object.");
        ...
        sprite.GetComponent<SpriteAnimator>().mIsPaused = false;
    }

    public void RestartFrame(GameObject sprite, int nbRestart, int atFrame)
    {
        sprite.GetComponent<SpriteAnimator>().mIsPaused = false;
        sprite.GetComponent<SpriteAnimator>().RestartFrame(nbRestart, atFrame);
    }

    public bool IsPaused(GameObject sprite)
    {
        return sprite.GetComponent<SpriteAnimator>().mIsPaused;
    }
}
